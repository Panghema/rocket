...
void TinyPBCoder::decode(std::vector<AbstractProtocol::s_ptr>& out_messages, TcpBuffer::s_ptr buffer) {
    while(1) {
        ...
        // 遍历buffer找到PB_START，找到之后，解析出整包的长度，然后得到结束符的位置
        // 判断是否为PB_END，如果没找到，则说明这一条没发全
        ...
        if (parse_succ) {
            ...
            if (msg_id_len_index >= end_index) {
                ...
                ERRORLOG("prase error, msg_id_len_index[%d] > end_index[%d]", msg_id_len_index, end_index);
                continue;
            }
            ...
            if (method_name_len_index >= end_index) {
                message->parse_success = false;
                ERRORLOG("prase error, method_name_len_index[%d] > end_index[%d]", method_name_len_index, end_index);
                continue;
            }
            ...
            if (err_code_index >= end_index) {
                message->parse_success = false;
                ERRORLOG("prase error, err_code_index[%d] > end_index[%d]", err_code_index, end_index);
                continue;
            }
            ...
            if (error_info_len_index >= end_index) {
                message->parse_success = false;
                ERRORLOG("prase error, error_info_len_index[%d] > end_index[%d]", error_info_len_index, end_index);
                continue;
            }
...
const char* TinyPBCoder::encodeTinyPB(std::shared_ptr<TinyPBProtocol> message, int& len) {
    ...
    *tmp = TinyPBProtocol::PB_START;
    ...
    int err_code_net = htonl(message->m_err_code);
