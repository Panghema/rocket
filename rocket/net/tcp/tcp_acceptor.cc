...
TcpAcceptor::TcpAcceptor(NetAddr::s_ptr local_addr) : m_local_addr(local_addr) {
    ...
    // reactor要求全是异步，所以要非阻塞
    // 如果阻塞的话就会出现你要读的长度大于有的长度，就阻塞在那里了
    ...
    if (bind(m_listenfd, m_local_addr->getSockAddr(), len) != 0) {
        ERRORLOG("bind error, error, errno=%d, error=%s", errno, strerror(errno));
        exit(0);
    }
...
std::pair<int, NetAddr::s_ptr> TcpAcceptor::accept() {
    if (m_family == AF_INET) {
        ...
        INFOLOG("A client have been accepted succ, peer addr [%s]", peer_addr->toString().c_str());
